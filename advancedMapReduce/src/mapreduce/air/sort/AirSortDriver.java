 ...
 *     => run메소드를 직접 호출 할 수 없고 ToolRunner라는 헬퍼클래스를 
 *        이용해서호출
 *   
 */
public class AirSortDriver extends Configured 
									implements Tool{
	@Override
	public int run(String[] optionlist) throws Exception {
		//run메소드는 사용자가 입력한 모든 옵션에 대한 정보를 String[]
		//로 전달받는다.
		//-D를 입력하고 설정한 옵션과 사용자가 입력한 명령행매개변수를 분리하여
		//관리해야 한다.
		//getRemainingArgs()를 이용하여 
		//공통옵션(-D와 입력한 값 이외의 입력값)
		...
		//Shuffle를 사용할 때 사용자정의 클래스가 실행되도록 등록
