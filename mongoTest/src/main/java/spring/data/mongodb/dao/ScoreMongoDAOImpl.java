...
//spring-data에서 지원되는 Repository와 MongoTemplate 클래스를
...
public class ScoreMongoDAOImpl implements ScoreMongoDAO {
	...
	@Autowired
	ScoreRepository scoreRepository; //페이징처리를 편하게 하기 위해서
	
	//mongodb를 연동하기 위한 기능을 제공
	//(spring-data의 mongodb 라이브러리에서 지원하는 기능
	@Autowired
	...
	public List<ScoreDTO> findCriteria(String key, String value) {
		...
		/*
		Query 클래스의 addCriterial 메소드와 Criteria 캘르스의 where 메소드의 활용
		...
		*/
		...
		criteria.andOperator(
				...
				Criteria.where("addr").is("인천")); //원래는 입력받아야 함
	...
	//mongoDB가 json으로 모든 작업을 처리하므로 key, value로 저건을 정의
	...
	public ScoreDTO findById(String key, String value) {
		...
		criteria.is(value);
		// 3. Criteria를 이용해서 Query 객체를 작성
		Query query = new Query(criteria);
		// 4. MongoTemplate 클래스의 메소드를 호출하며 Query 객체를 
		// 매개변수로 전달
	...
	public List<ScoreDTO> findAll(int pageNo) {
		...
		//호출하면 srping-data내부에서 페이징 처리가 된 객체를 전달받을 수 있다
